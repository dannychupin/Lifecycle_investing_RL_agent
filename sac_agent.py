...
"""
    ...
    State space is continuous,
        dim_state =     dim_action (number of asset returns)
                        + dim_ancilla (number of macro variables, like returns to cpi, gdp, ...)
                        + 1 (wealth)
...
class ActorNetworkGaussian(nn.Module):
    ...
    def forward(self, state):
        ...
        if not torch.isfinite(state).all():
            raise RuntimeError("NaNs/Infs in state")

        if not torch.isfinite(h).all():
            raise RuntimeError("NaNs/Infs in trunk output h")

        if not torch.isfinite(mean).all():
            raise RuntimeError("NaNs/Infs in mean")

        if not torch.isfinite(logstd).all():
            raise RuntimeError("NaNs/Infs in logstd")
...
class ReplayBuffer:
    ...
    def get_batch(self):
        ...
        # Option #1: uniformly random (with replacement? without? make sure there is a wait time before sampling)
        batch = np.random.choice(memory_lower_bound, self.batch_size, replace=self.replace)
        # Option #2: contiguous
        # start = np.random.choice(memory_lower_bound - self.batch_size, 1)[0]
        # batch = [start + k for k in range(self.batch_size)]
...
class SoftActorCritic:
    ...
    def __init__(self, state_dimension,
        ...
        self.log_alpha = nn.Parameter(torch.tensor(np.log(alpha_initial)))
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            # remember: for Q update, no need to rsample (and not within no_grad())
            ...
            continuing_piece = min_of_target_values_batch - alpha * new_action_log_prob_batch

            target_future_rewards_batch = new_reward_batch + self.gamma * (1 - done_batch) * continuing_piece
        ...
        # 2) IMPORTANT: .rsample() alternative actions on (old) states
        ...
        action_log_prob_batch = action_distribution_batch.log_prob(action_batch)

        # 3) use updated Q's to compute their minimum.
        Q1_value_batch = self.Q1.forward(state_batch, action_batch).squeeze()
        Q2_value_batch = self.Q2.forward(state_batch, action_batch).squeeze()
        min_of_Q_value_batch = torch.min(Q1_value_batch, Q2_value_batch)

        # 4) calculate loss, and take a step
        policy_loss = torch.mean(alpha * action_log_prob_batch - min_of_Q_value_batch)
        ...
        log_alpha_loss = -(self.log_alpha * (action_log_prob_batch.detach() + self.H_target)).mean()
